<section>  
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">  
        ...
        <div class="form-group">  
            ...
            <x-text-input id="name" name="name" type="text" class="text-input" :value="old('name', $user->name)" required autofocus autocomplete="name" />  
            <x-input-error class="error" :messages="$errors->get('name')" />  
        </div>  

        <div class="form-group">  
            <x-input-label for="email" :value="__('Email')" />  
            <x-text-input id="email" name="email" type="email" class="text-input" :value="old('email', $user->email)" required autocomplete="username" />  
